...
wc = getWordCount(fName)
def wordProbability(chosenWord, wordCount) : #wc instead of wordCount
    wordOccurences = wordCount[chosenWord]
    totalWords = 0
    for chosenWord in wordCount:
        totalWords = totalWords + wordCount[chosenWord]
    chosenWordProbability = wordOccurences / totalWords
    return chosenWordProbability
    # count the total number of words in the histogram
    # final the count for this word
    # return that count / total
...
def getRandomWord(histogram) :
    histogramWordList = list(histogram.keys())
    randomWord = random.choice(histogramWordList)
    print(randomWord)
...
fName = 'hamlet.txt'
wc = getWordCount(fName)
def getRandomSentence(histogram) :
    allHistogramWords = []
    for word, freq in histogram.items():
        allHistogramWords.extend([word] * freq)
    return random.choices(allHistogramWords, k=10) #k = number of items chosen
...
listOfWords = ['thou', 'thus', 'thee', 'thine', 'thyself', 'hamlet', 'horatio', 'ophelia']
...
fName = 'hamlet.txt'
mWC = getMarkovWordCount(fName)
def buildMarkovHistogram(markovWordCount) : #mWC as input
    markovHist = {}
...
fname = 'hamlet.txt'
def makeMarkovGenerator(fname) :
    mWC = getMarkovWordCount(fname)
    markovHist = buildMarkovHistogram(mWC)
    start = getRandomWord(mWC)
    for i in range(15) :
        firstWord = markovHist[start]
